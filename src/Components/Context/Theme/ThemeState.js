...
function ThemeState(props) {
  ...
  const handleExpand = () => {
    if (NewsShrink === '') {
      ...
      localStorage.setItem('bodyExpand', JSON.stringify(!NewsShrink));
    };

    useEffect(() => {
     const bodyExpand = JSON.parse(localStorage.getItem('bodyExpand'))
     setNewsShrink(bodyExpand?  'mainShrink' : '');
    }, [])
